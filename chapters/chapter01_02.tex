...
\hline
\textbf{Intuition Track} & \textbf{Formal Track} \\
\hline
Think of a machine that encodes data. If two different inputs can produce the same output, decoding is ambiguous and inversion fails. & We need injectivity: \(f(x_1)=f(x_2)\Rightarrow x_1=x_2\). Then define \(f^{-1}\) on the range of \(f\), and use \(f^{-1}(f(x))=x\), \(f(f^{-1}(x))=x\). \\
\hline
\end{tabular}
\end{center}
...
\textbf{Intuition Track} & \textbf{Formal Track} \\
\hline
Angle recovery appears in surveying and navigation, but periodic trig graphs repeat outputs. We must pick one branch to get one angle per value. & Define \(\arcsin,\arccos,\arctan\) by restricting \(\sin,\cos,\tan\) to one-to-one intervals. State domain/range explicitly and apply composition identities only on valid intervals. \\
\hline
\end{tabular}
\end{center}
...
\textbf{Intuition Track} & \textbf{Formal Track} \\
\hline
Average speed over a short interval approximates instantaneous speed. The closer the interval is to zero, the better the approximation. & Use \(\lim_{x\to a}f(x)=L\): for \(x\) near \(a\) (with \(x\ne a\)), \(f(x)\) is near \(L\). The function value at \(a\) may differ or be undefined. \\
\hline
\end{tabular}
\end{center}
...
\textbf{Intuition Track} & \textbf{Formal Track} \\
\hline
Approaching a threshold from left and right can show different behaviors, as in piecewise systems or saturation effects. & Define \(\lim_{x\to a^-}f(x)\), \(\lim_{x\to a^+}f(x)\), and infinite limits. Two-sided limits require agreement of left/right limits; vertical asymptotes come from one-sided infinite behavior. \\
\hline
\end{tabular}
\end{center}
...
\textbf{Intuition Track} & \textbf{Formal Track} \\
\hline
Calculation is a decision process: try direct substitution first, then simplify if you get \(0/0\), and compare when oscillation appears. & Apply limit laws, direct substitution for polynomials/rational functions (when defined), algebraic simplification (factoring/rationalization), one-sided analysis for piecewise functions, and the squeeze theorem. \\
\hline
\end{tabular}
\end{center}
...
\textbf{Intuition Track} & \textbf{Formal Track} \\
\hline
The phrase ``close enough'' must become a rule that works for every required accuracy. & For every \(\varepsilon>0\), find \(\delta>0\) such that \(0<|x-a|<\delta \Rightarrow |f(x)-L|<\varepsilon\). This is the core quantifier pattern for rigorous proofs. \\
\hline
\end{tabular}
\end{center}
