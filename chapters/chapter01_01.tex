...
\begin{remark}[Problem-Driven Opener]
\textbf{Guiding question.} If a process returns an output \(y\), when can we recover a unique input \(x\)?

\textbf{Why this matters.} In real systems, decoding ID numbers, converting units, and reverse calibration all rely on inverse maps. This section gives the exact condition that makes such reversal possible.
\end{remark}
...
A real-valued function is one-to-one if and only if no horizontal line intersects its graph more than once.
\end{theorem}
...
% manual page break removed to let floats place naturally
...
\begin{remark}[Problem-Driven Opener]
\textbf{Guiding question.} Why does \(\sin x\) need a domain restriction before we can define its inverse?

\textbf{Why this matters.} Angle reconstruction appears in surveying, robotics pose estimation, and signal phase analysis. The branch choice is not cosmetic; it determines the answer students get.
\end{remark}
...
\begin{remark}
Different textbooks may choose different principal-value branches for \(\arcsec\) and \(\arccsc\). In this chapter we use the ranges above, and all identities and differentiation formulas should be interpreted with this convention.
\end{remark}

\section{The Limit of a Function}

\begin{remark}[Problem-Driven Opener]
\textbf{Guiding question.} How can we measure speed at one instant if velocity is computed from two times?

\textbf{Why this matters.} Limits formalize the transition from average change to instantaneous change. Historically, this idea developed from Newton--Leibniz methods to modern rigor by Cauchy and Weierstrass.
\end{remark}
...
\begin{remark}[Problem-Driven Opener]
\textbf{Guiding question.} What if a system behaves differently from the left and from the right of a threshold?

\textbf{Why this matters.} Piecewise models, queueing behavior near saturation, and singular formulas in physics all require one-sided and infinite-limit language.
\end{remark}
...
\begin{remark}[Problem-Driven Opener]
\textbf{Guiding question.} After understanding the idea of limit, how do we compute one efficiently by hand?

\textbf{Why this matters.} Most first-year calculus work is not proving from first principles every time. This section gives a reliable workflow: substitute, simplify, and squeeze.
\end{remark}
...
\begin{example}
The greatest integer function \(\lfloor x \rfloor\) is defined to be the largest integer less than or equal to \(x\). Evaluate
\[
...
\begin{remark}[Problem-Driven Opener]
\textbf{Guiding question.} What does ``\(x\) close to \(a\)'' mean in a way that can be proved?

\textbf{Why this matters.} The \(\varepsilon\)-\(\delta\) definition turns intuition into a verifiable statement. This is the foundation used in advanced analysis and modern mathematical modeling.
\end{remark}
